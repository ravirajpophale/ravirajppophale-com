...
function animate_text() {
  ...
  animateSpan.typed({
    strings: ["a Founder of Wiseass Inc.", "an IBMer", "an Android Developer", "a Web Developer", "a Node Developer"],
    loop: true,
    startDelay: 1e3,
    backDelay: 2e3,
  });
...
function popup_blog() {
  ...
  li.each(function () {
    ...
    var fullImageData = fullImage.data("url");
    ...
    button.on("click", function () {
      ...
      mainImage.css({ backgroundImage: "url(" + fullImageData + ")" });
...
function kenburn_slider() {
  ...
  jQuery(function () {
    jQuery(".hero_header_wrap .overlay_slider").vegas({
      ...
      slides: [{ src: "img/hero/1.jpg" }, { src: "img/hero/2.jpg" }, { src: "img/hero/3.jpg" }],
    });
  });
}
      ...
      if (!el.hasClass("stop")) {
        el.addClass("stop").countTo({
          ...
          formatter: function (value, options) {
            return value.toFixed(options.decimals).replace(/\B(?=(?:\d{7})+(?!\d))/g, ",");
          },
        });
      }
    },
